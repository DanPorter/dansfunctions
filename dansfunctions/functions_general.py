...
def inline_help(func, markdown=False):
...
def list_methods(object, include_special=False, markdown=False):
    """Return list of methods (functions) in class object"""
    methods = get_methods(object, include_special)
    if markdown:
        return '\n\n'.join([inline_help(getattr(object, fun), markdown) for fun in methods])
